...
export default function Calculator() {
  ...
  const parseAndCalc = async () => {
    setError('')
    if (!signal.trim()) { setError(t('calc.errSignal')); return }
    ...
    setLoading(true)
    try {
      const parsed = await API.parse(signal)
      const spec   = getInstrumentDefaults(parsed.instrument)
      const ps  = pipSize  !== '' ? parseFloat(pipSize)  : spec.pip_size
      const pv  = pipValue !== '' ? parseFloat(pipValue) : spec.pip_value
      const cs  = contractSize !== '' ? contractSize : String(spec.contract_size)
      setContractSize(cs)
      setPipSize(String(ps))
      setPipValue(String(pv))
      const result = await API.calculate({
        direction:    parsed.direction,
        instrument:   parsed.instrument,
        entry_avg:    parsed.entry_avg,
  ...
  return (
    <>
      <div className="page-header">
        <h2><i className="bi bi-calculator me-2 text-accent"></i>{t('calc.titleParsing')}</h2>
      </div>
          ...
          <div className="col-xl-5">
            <div className="card">
              <div className="card-body">
                <label className="form-label fw-semibold">{t('calc.paste')}</label>
                <textarea
                  className="form-control signal-textarea"
                  rows="10"
                  placeholder={"BUY XAU/USD\nEntry: 4150 - 4154.6\nSL: 4136\nTP1: 4159.2\nTP2: 4166.1\nTP3: 4180\nTP4: 4200"}
                  value={signal}
                ...
                <div className="row g-2 mt-3">
                  <div className="col-6">
                    ...
                    <input type="number" className="form-control" value={deposit} onChange={e => setDeposit(e.target.value)} min="1" />
                  </div>
                  <div className="col-6">
                    <label className="form-label small">{t('calc.risk')}</label>
                    <input type="number" className="form-control" value={risk} onChange={e => setRisk(e.target.value)} min="0.01" max="100" step="0.1" />
                  </div>
                  <div className="col-6">
                ...
                <div className="row g-2 mt-1">
                  <div className="col-4">
                    ...
                    <input type="number" className="form-control form-control-sm" step="any" min="0"
                      placeholder="auto" value={contractSize} onChange={e => setContractSize(e.target.value)} />
                  </div>
                  <div className="col-4">
                    <label className="form-label small">{t('calc.pipSize')}</label>
                    <input type="number" className="form-control form-control-sm" step="any" min="0"
                      placeholder="auto" value={pipSize} onChange={e => setPipSize(e.target.value)} />
                  </div>
                  <div className="col-4">
                    <label className="form-label small">{t('calc.pipValue')}</label>
                    <input type="number" className="form-control form-control-sm" step="any" min="0"
                      placeholder="auto" value={pipValue} onChange={e => setPipValue(e.target.value)} />
                  </div>
                </div>

                <div className="d-grid mt-3">
                  <button className="btn btn-accent btn-lg" onClick={parseAndCalc} disabled={loading}>
                    {loading
                      ? <><span className="spinner-border spinner-border-sm me-1"></span>{t('calc.calculating')}</>
                      : <><i className="bi bi-lightning-charge-fill me-1"></i>{t('calc.parseBtn')}</>}
                  </button>
                </div>
            ...
            {!calc ? (
              <div className="card h-100 d-flex align-items-center justify-content-center text-muted" style={{ minHeight: 260 }}>
                <div className="text-center">
                  ...
                  {t('calc.placeholderParse')} <strong>{t('calc.parseBtn')}</strong>
                </div>
              </div>
            ) : (
              <>
                    ...
                    <table className="table table-dark table-hover mb-0">
                      ...
                      <tbody>
                        {calc.take_profit_results.length
                          ...
                          : <tr><td colSpan="6" className="text-muted text-center">{t('calc.noTPsFound')}</td></tr>}
                      </tbody>
                    </table>
                  </div>
                </div>
                  ...
                  <>
                    ...
                    {calc.positions.some(p => p.clamped_to_min) && (
                      <div className="alert alert-danger">
                        {t('calc.warnSplitMin', { min: calc.positions[0].lot, risk: fmtNum(calc.risk_amount) })}
                      </div>
                    )}
                  </>
                )}
